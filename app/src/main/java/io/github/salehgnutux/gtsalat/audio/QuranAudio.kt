...
object QuranAudio {
    ...
    /** تلاوةٌ آية-بآية متزامنة مع النصّ (الوضع النصّيّ). */
    fun playAyat(ctx: Context, surah: Int, surahName: String, verses: Int, reciter: Reciter, startAyah: Int = 1) =
        start(ctx, QuranAudioService.MODE_AYAH, surah, surahName, verses, reciter.everyayah, reciter, startAyah)

    /** تلاوة سورةٍ كاملة (القرآن المسموع). */
    fun playSurah(ctx: Context, surah: Int, surahName: String, reciter: Reciter) =
        start(ctx, QuranAudioService.MODE_SURAH, surah, surahName, 0, reciter.mp3quran, reciter, 1)

    private fun start(
        ctx: Context, mode: String, surah: Int, surahName: String, verses: Int,
        folder: String, reciter: Reciter, startAyah: Int,
    ) {
        val i = Intent(ctx, QuranAudioService::class.java).apply {
            ...
            putExtra(QuranAudioService.EXTRA_RECITER_ID, reciter.id)
            putExtra(QuranAudioService.EXTRA_RECITER_NAME, reciter.ar)
