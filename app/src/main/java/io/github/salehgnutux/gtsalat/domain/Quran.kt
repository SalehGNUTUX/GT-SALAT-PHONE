...
object Quran {
     ...
     */
    // تشكيلٌ وعلاماتٌ قرآنيّة: حركات + علاماتٌ عُليا صغيرة + ألفٌ خنجريّة + علامات التلاوة + تطويل + BOM.
    private val diacritics = "[\\u0610-\\u061A\\u064B-\\u065F\\u0670\\u06D6-\\u06ED\\u0640\\uFEFF]".toRegex()
    private val spaces = "\\s+".toRegex()

    /**
     * تطبيعٌ عربيٌّ للبحث: يزيل التشكيل وعلامات التلاوة، ويوحّد صور الحروف، **ويحذف الألف كلّها**
     * فيصبح البحث غير حسّاسٍ للتشكيل ولا للألف (يعالج ألف الخنجريّة «جنّٰت» ومواضع حذف/إثبات الألف).
     */
    fun normalize(s: String): String = s
        ...
        .replace("ء", "")
        .replace("ا", "")   // إزالة الألف لجعل البحث غير حسّاسٍ لها
        .replace(spaces, " ")
        .trim().lowercase()
}
